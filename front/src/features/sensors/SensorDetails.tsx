...
import type { RealSensor } from '../../hooks/useSensors';

const SensorDetails: React.FC = () => {
  const { sensors, loading, error } = useRealSensors();
  const [filter, setFilter] = useState<'all' | 'température' | 'humidité' | 'mouvement'>('all');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5; 

  const filteredData = filter === 'all' ? sensors : sensors.filter((item) => item.type === filter as RealSensor['type']);
